...
func TestPostgreSQLServiceIntegration(t *testing.T) {
	...
	cfg := &config.Config{
		PostgreConf: config.PostgreConf{
			PostgresMaster: "postgres://postgres:damir@127.0.0.1:5432/gamesparks_db?sslmode=disable",
			PostgresReplicas: []string{
				"postgres://postgres:damir@127.0.0.1:5432/gamesparks_db?sslmode=disable",
			},
		},
	}
