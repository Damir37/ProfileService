...
type User struct {
	...
	StreamKey   string    `boil:"stream_key" json:"stream_key" toml:"stream_key" yaml:"stream_key"`
	Picture     string    `boil:"picture" json:"picture" toml:"picture" yaml:"picture"`
...
var UserWhere = struct {
	...
	StreamKey   whereHelperstring
	Picture     whereHelperstring
	...
	IP:          whereHelperstring{field: "\"users\".\"ip\""},
	IsActivated: whereHelperbool{field: "\"users\".\"is_activated\""},
	IsAdmin:     whereHelperbool{field: "\"users\".\"is_admin\""},
	StreamKey:   whereHelperstring{field: "\"users\".\"stream_key\""},
	Picture:     whereHelperstring{field: "\"users\".\"picture\""},
...
var UserRels = struct {
	UsersEconomies string
	UsersStats     string
}{
	UsersEconomies: "UsersEconomies",
	UsersStats:     "UsersStats",
}

// userR is where relationships are stored.
type userR struct {
	UsersEconomies UsersEconomySlice `boil:"UsersEconomies" json:"UsersEconomies" toml:"UsersEconomies" yaml:"UsersEconomies"`
	UsersStats     UsersStatSlice    `boil:"UsersStats" json:"UsersStats" toml:"UsersStats" yaml:"UsersStats"`
}
...
func (r *userR) GetUsersEconomies() UsersEconomySlice {
	if r == nil {
		return nil
	}
	return r.UsersEconomies
}

func (r *userR) GetUsersStats() UsersStatSlice {
	if r == nil {
		return nil
	}
	return r.UsersStats
...
var (
	userAllColumns            = []string{"id", "username", "email", "password", "ip", "is_activated", "is_admin", "stream_key", "picture", "updated_at", "created_at"}
	userColumnsWithoutDefault = []string{"username", "email", "password", "ip", "stream_key", "picture"}
	userColumnsWithDefault    = []string{"id", "is_activated", "is_admin", "updated_at", "created_at"}
...
// UsersEconomies retrieves all the users_economy's UsersEconomies with an executor.
func (o *User) UsersEconomies(mods ...qm.QueryMod) usersEconomyQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"users_economy\".\"user_id\"=?", o.ID),
	)

	return UsersEconomies(queryMods...)
}

// UsersStats retrieves all the users_stat's UsersStats with an executor.
func (o *User) UsersStats(mods ...qm.QueryMod) usersStatQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"users_stats\".\"user_id\"=?", o.ID),
	)

	return UsersStats(queryMods...)
}

// LoadUsersEconomies allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUsersEconomies(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`users_economy`),
		qm.WhereIn(`users_economy.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load users_economy")
	}

	var resultSlice []*UsersEconomy
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice users_economy")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on users_economy")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for users_economy")
	}
	...
	if singular {
		object.R.UsersEconomies = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &usersEconomyR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.UsersEconomies = append(local.R.UsersEconomies, foreign)
				if foreign.R == nil {
					foreign.R = &usersEconomyR{}
				}
...
// LoadUsersStats allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUsersStats(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`users_stats`),
		qm.WhereIn(`users_stats.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load users_stats")
	}

	var resultSlice []*UsersStat
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice users_stats")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on users_stats")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for users_stats")
	}
	...
	if singular {
		object.R.UsersStats = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &usersStatR{}
			}
			foreign.R.User = object
		}
		return nil
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.UsersStats = append(local.R.UsersStats, foreign)
				if foreign.R == nil {
					foreign.R = &usersStatR{}
				}
				foreign.R.User = local
...
// AddUsersEconomies adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UsersEconomies.
// Sets related.R.User appropriately.
func (o *User) AddUsersEconomies(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UsersEconomy) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			UsersEconomies: related,
		}
	} else {
		o.R.UsersEconomies = append(o.R.UsersEconomies, related...)
	}
...
// AddUsersStats adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UsersStats.
// Sets related.R.User appropriately.
func (o *User) AddUsersStats(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UsersStat) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
	...
	if o.R == nil {
		...
		o.R.UsersStats = append(o.R.UsersStats, related...)
	}
